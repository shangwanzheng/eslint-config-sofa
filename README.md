...
```bash
npm install --save-dev eslint @babel/eslint-parser eslint-config-sofa
```
...
```bash
npm install --save-dev eslint typescript @typescript-eslint/parser @typescript-eslint/eslint-plugin eslint-config-sofa
```
...
目前已经不包含所有样式相关的规则了，故不需要引入 `eslint-config-prettier`。只需要安装 `prettier` 及相关 VSCode 插件即可。

下面给出一个 sofaTeam 使用的 `.prettierrc.js` 配置，仅供参考：
